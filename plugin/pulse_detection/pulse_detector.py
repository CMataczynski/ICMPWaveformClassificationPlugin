...
class Segmenter:
    ...
    def _detect_pulses_in_signal(self,
        x: np.ndarray,
        fs: float
    ) -> np.ndarray:
        dx = sp_sig.detrend(x)
        ...
        signal_peaks = self._detect_peaks_troughs(-fx, max_scale=fs)
    ...
    def split_pulses(self, icp: np.ndarray, time: np.ndarray, fs_hat: float, mean_time: bool = False) -> List[np.ndarray]:
        peks = self._detect_pulses_in_signal(icp, fs_hat)
        peks = np.concatenate(([0], peks, [len(icp) - 1]))
